...
def main():
    ...
    parser.add_argument('--repeat_num', '-r', help='Number of repeatly using each loaded h5 list', type=int, default=1)

    args = parser.parse_args()
    print(args)

    root = args.folder if args.folder else '../../data/semantic3d/downsampled'

    splits = ['train', 'val', 'test', 'test_reduced']
    ...
    test_h5 = split_filelists['test_reduced']
    test_list = os.path.join(root, 'test_reduced_files.txt')
    print('{}-Saving {}...'.format(datetime.now(), test_list))
    with open(test_list, 'w') as filelist:
        for filename_h5 in test_h5:
            filelist.write(filename_h5)
