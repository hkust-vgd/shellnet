...
def main():
    ...
    parser.add_argument('--load_ckpt', '-l', default='log/seg/shellconv_seg_semantic3d_2019-08-04-15-52-37/ckpts/epoch-79', help='Path to a check point file for load')
    ...
    parser.add_argument('--save_ply', '-s', help='Save results as ply', default=True)
    ...
    with tf.Session() as sess:
        ...
        folder = os.path.dirname(setting.filelist_test)
        filenames = [os.path.join(folder, line.strip()) for line in open(setting.filelist_test)]
        for filename in filenames:
            filename_pred = filename[:-3] + '_pred.h5'
            if os.path.isfile(filename_pred):
                continue
